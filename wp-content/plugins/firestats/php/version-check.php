...
function fs_lazy_get_latest_version_info($url, $type, $default_check_interval, &$error, $force_check = false)
{
	...
	$version_check_enabled = fs_get_option($type."_version_check_enabled",'true');
	if ($version_check_enabled != 'false' || $force_check)
	{
		$last_version_check_time = (int)fs_get_option($type.'_last_version_check_time');
		$version_check_interval = (int)fs_get_option($type.'_version_check_interval',$default_check_interval);
		...
		if ($now > $d || $force_check)
		{
			...
			fs_update_option($type."_version_check_enabled",'false');
			...
			fs_update_option($type.'_last_version_check_time',$now);

			// the fetch was successful, we can re-activate the version check
			fs_update_option($type."_version_check_enabled",'true');
			...
			if ($info != null)
			{
				fs_update_option($type.'_last_version_info_on_server',serialize($info));
			}
			return $info;
		}
		else
		{
			...
			$info = fs_get_option($type.'_last_version_info_on_server');
			if ($info)
			{
...
{
	$name = dirname(__FILE__)."/ip2c/db.version";
...
{
	return fs_get_option('botlist_version_hash','');
}
