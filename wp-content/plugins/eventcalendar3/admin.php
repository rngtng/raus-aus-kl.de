...
class ec3_Admin
{
  ...
  function filter_admin_head()
  {
    ...
    Ec3EditForm.start_of_week=<?php echo intval( get_settings('start_of_week') ); ?>;
    // --></script>
      ...
      if($ec3->event_category)
      {
        ...
        if($results)
          foreach($results as $r)
          {
            $wpdb->query(
              "UPDATE $wpdb->posts
               SET post_date    =LEAST(NOW(),post_modified),
                   post_date_gmt=LEAST(NOW(),post_modified_gmt)
               WHERE ID=$r->ID"
            );
          }

      } // end if($ec3->event_category)
    } // end if(!$table_exists)
...
// Always hook in to the admin_menu - it's required to allow users to
// set things up.
