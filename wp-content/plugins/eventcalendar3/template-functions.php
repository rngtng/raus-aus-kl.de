<?php
/*
Copyright (c) 2005 2006, Alex Tingle.  $Revision: 240 $
...
function ec3_check_installed($title)
{
        ...
        {?>
                ...
                get_settings('home');?>/wp-admin/options-general.php?page=ec3_admin">
...
define('EC3_DEFAULT_DATE_FORMAT',   'l, j. F');
	...
	$q = "SELECT DISTINCT
		...
		$today = mktime(23, 59, 0 );
                foreach($calendar_entries as $entry)
                {
                        ...
                        if( ec3_to_time($entry->start) < $today ) $data['MONTH'] = 'aktuell';
                        if((!$current_month || $current_month!=$data['MONTH']) && $template_month)
                        {
                                if($current_date)   echo "</ul></li>\n";
                                if($current_month)  echo "</ul></li>\n";
                                $str = ( ec3_to_time($entry->start) < time() ) ? 'Aktuell' : ec3_format_str($template_month,$data);
                                echo "<li class='ec3_list ec3_list_month'>$str\n<ul>\n";
                        ...
                        if( ec3_to_time($entry->start) < $today ) $data['DATE'] = mktime( 22 );
                        if((!$current_date || $current_date!=$data['DATE']) && $template_day)
                        {
                                if($current_date) echo "</ul></li>\n";
                                $str = ( ec3_to_time($entry->start) < $today ) ? 'Heute:' : ec3_format_str($template_day,$data);
                                echo "<li class='ec3_list ec3_list_day'>$str\n<ul>\n";
...
define('EC3_DEFAULT_FORMAT_SINGLE','%s <br>');
define('EC3_DEFAULT_FORMAT_RANGE','%1$s - %2$s');
